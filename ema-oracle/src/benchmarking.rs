...
benchmarks! {
    ...
    on_finalize_insert_one_token {
        ...
        let entry = OracleEntry::new(Price::from((liquidity_asset_in, liquidity_asset_out)), Volume::from_a_in_b_out(amount_in, amount_out), Liquidity::new(liquidity_asset_in, liquidity_asset_out), block_num);
    ...
    on_liquidity_changed_multiple_tokens {
        ...
        frame_system::Pallet::<T>::set_block_number(block_num);
        EmaOracle::<T>::on_initialize(block_num);
        let entry = OracleEntry::new(Price::from((liquidity_asset_a, liquidity_asset_b)), Volume::from_a_in_b_out(amount_a, amount_b), Liquidity::new(liquidity_asset_a, liquidity_asset_b), block_num);
        for i in 0 .. b {
            let asset_a = i * 1_000;
            let asset_b = asset_a + 500;
            assert_ok!(OnActivityHandler::<T>::on_trade(SOURCE, asset_a, asset_b, amount_a, amount_b, liquidity_asset_a, liquidity_asset_b));
    ...
    verify {
        assert_ok!(*res.borrow());
        let liquidity_entry = OracleEntry::new(Price::from((liquidity_asset_a, liquidity_asset_b)), Volume::default(), Liquidity::new(liquidity_asset_a, liquidity_asset_b), block_num);
