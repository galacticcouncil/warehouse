    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            for block in (1..=200).step_by(1) {
                let (previous_asset_fee, previous_protocol_fee, _) = crate::AssetFee::<Test>::get(HDX).unwrap();
                ...
                match asset_volume.amount_out.cmp(&asset_volume.amount_in) {
                    Ordering::Less => {
                        assert!(previous_asset_fee >= asset_fee);
                        assert!(previous_protocol_fee <= protocol_fee);
                    }
                    Ordering::Equal => {
                        assert_eq!(previous_asset_fee, asset_fee);
                        assert_eq!(previous_protocol_fee, protocol_fee);
                    }
                    Ordering::Greater => {
                        assert!(previous_asset_fee <= asset_fee);
                        assert!(previous_protocol_fee >= protocol_fee);
